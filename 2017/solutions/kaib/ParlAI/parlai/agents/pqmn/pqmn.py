...
class PqmnAgent(Agent):
    ...
    def report(self):
        return (
            '[train] updates = %d | train loss (avg) = %.2f | train loss = %.2f | exs = %d' %
            (self.model.updates, self.model.train_loss.avg, self.model.train_loss.val, self.n_examples)
            )
