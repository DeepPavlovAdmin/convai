...
data_pos = ['./data/squad_train_pq.txt', 
...
x_text, y, match = data_helpers.load_data_and_labels_em(data_pos, data_neg)
...
# Build vocabulary
for m_data in match:
    m_zero = np.zeros((max_document_length - len(m_data),), dtype=np.float)
    m_temp = np.concatenate([m_data, m_zero], 0)
    m_a.append(m_temp)


m = np.array(m_a)
#x_match = np.array(match)
#x = np.concatenate((x_word, x_match[:, None]) , 1)
...
m_train, m_dev = m_shuffled[:dev_sample_index], m_shuffled[dev_sample_index:]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_batch, m_batch):
            """
        ...
        def dev_step(x_dev, y_dev, m_dev, writer=None):
            """
            ...
            for idx in range(0, int((len(x_dev)-1)/batch_size + 1)):
                ...
                feed_dict = {
                  ...
                  cnn.input_m: m_dev[idx*batch_size:end_idx],
        ...
        batches = data_helpers.batch_iter(
            list(zip(x_train, y_train, m_train)), FLAGS.batch_size, FLAGS.num_epochs)
        ...
        for batch in batches:
            ...
            train_step(x_batch, y_batch, m_batch)
            current_step = tf.train.global_step(sess, global_step)
            if current_step % FLAGS.evaluate_every == 0:
                print("\nEvaluation:")
                dev_step(x_dev, y_dev, m_dev, writer=dev_summary_writer)        
                print("")
