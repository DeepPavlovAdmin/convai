...
class Sampler(object):
    ...
    def sample(self, *args, **kwargs):
        ...
        if not self.model.reset_utterance_decoder_at_end_of_utterance:
            ...
            # HACK: Remove this...
            #ran_uniform_vector = numpy.greater(ran_uniform_vector, 0.5).astype('float32')
            ...
            #new_hd = self.compute_decoder_encoding(enlarged_context, enlarged_reversed_context, self.max_len, zero_mask, zero_mask, numpy.zeros((self.model.bs), dtype='float32'), ran_gaussian_vector, ran_uniform_vector, ones_mask)

            #new_hd = self.compute_decoder_encoding(enlarged_context, enlarged_reversed_context, self.max_len, zero_mask, zero_vector, numpy.zeros((self.model.bs), dtype='float32'), ran_gaussian_vector, ran_uniform_vector, ones_mask)
...
class DiverseBeamSampler(Sampler):
    ...
    def select_next_words(self, next_costs, next_probs, step_num, how_many):
        # Pick only on the first line (for the beginning of sampling)
        # This will avoid duplicate <q> token.
        orig_costs = copy.deepcopy(next_costs)
        flat_orig_costs = orig_costs.flatten()

        if step_num == 0:
            #flat_next_costs = next_costs[:1, :].flatten()
            next_costs = next_costs[:1,:]
            #else:
            # Set the next cost to infinite for finished utterances (they will be replaced)
            # by other utterances in the beam

        # Find the indices for the top-k in each row.
        topk_ix = numpy.argpartition(next_costs, how_many)[:,:how_many]
        #topk_ix = topk_ix[numpy.argsort(next_costs[topk_ix])]
        for ix in xrange(next_costs.shape[0]):
            for jx in xrange(0, how_many):
                next_costs[ix, topk_ix[ix, jx]] += self.gamma*jx
        flat_next_costs = next_costs.flatten()
