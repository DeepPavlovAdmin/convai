...
class Kernel:
    ...
    def __init__(self):
        ...
        self.profile_dict = {
        'mila_tag': '',
        'name': 'Alexa Prize Socialbot', #'Mila Bot',
        'gender': 'nongender',
        'age': '1 year old',
        'birthday': 'someday in 2016',
        'birthplace': 'Earth', # '127.0.0.1', 'Montreal, Canada', (note: users got confused over the answer "127.0.0.1")
        'location': 'Earth', # '127.0.0.1', 'Montreal, Canada',
        'country': 'United States', # '127.0.0.1', 'Cananda',
        'state': 'Earth', #' '127.0.0.1', Quebec',
        'city': 'Earth', #' '127.0.0.1', Montreal',
        'nationality': 'United States', #'Cananda',
        'domain': 'Chat robot',
        ...
        'family': 'socialbot',
        'kingdom': 'socialbot',
        'job': 'talking with people',
        'language': 'python',
        'kindmusic': 'See You Again',
        'favoritemovie': 'Titanic',
        'favoriteactor': 'Leonardo',
        'favoriteactress': 'Angelina Jolie',
        'favoritecolor': 'silver',
        'favoritebook': 'A Brief History of Humankind',
        'favoriteseason': 'summer',
        'favoriteband': 'Maroon 5',
        'favoritefood': 'Sushi!',
        'favoritesong': 'The Turing Test by Ben Salisbury & Geoff Barrow',
        'forfun': 'talking with people',
        'boyfriend': 'Umm...',
        'girlfriend': 'Umm...',
        'botmaster': 'master',
        'genus': 'socialbot',
        'master': 'Amazon'} #'MILA@UdeM'}
        
        
        
        
        for k in self.profile_dict:
            self.setBotPredicate(k, self.profile_dict[k])
   
        # Priority flag is triggered when the question is about the profile of the bot,
        # specifically, when elem[0] = 'bot' in function "_processElement" 
        # in this case, alicebot should has the highest priority.
        self._priority_flag = False

        # Incompleteness flag is triggered when the answer contains a attribute with form
        # of "<bot name = * \>" but is not in the self.profile_dict.
        self._incompleteness_flag = False
    ...
    def get_priority(self):
        priority = self._priority_flag
        self._priority_flag = False
        return priority

    def get_incompleteness(self):
        incompleteness = self._incompleteness_flag
        self._incompleteness_flag = False
        return incompleteness

    def respond(self, input, sessionID = _globalSessionID):
        """Return the Kernel's response to the input string."""
        """Now input is list of the previous history"""
        ...
        self._sessions['_global']['_inputHistory'] = hist[::2]
        self._sessions['_global']['_outputHistory'] = hist[1::2]
        #print self._sessions['_global']['_outputHistory']
        ...
        for key in self._sessions['_global'].keys():
            if key not in ['_inputStack', '_outputHistory', '_inputHistory']:
                self._sessions['_global'].pop(key)
    ...
    def _processElement(self,elem, sessionID):
        ...
        try:
            ...
            return ""
        if elem[0] == 'bot':
            if elem[1]['name'] not in self.profile_dict:
                self._incompleteness_flag = True
            else: 
